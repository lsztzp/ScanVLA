...
def scanvla_collect_fn(
    ...
    for example in instances:
        ...
        scanpath_x.append(example['fixation_x'])
        scanpath_y.append(example['fixation_y'])
        ref_encode.append(example['ref_encode'])
        ref_offset_mask.append(example['ref_offset_mask'])

    scanpath_x = torch.stack(scanpath_x)  #1
    scanpath_y = torch.stack(scanpath_y)
    ref_encode = torch.stack(ref_encode)
    ref_offset_mask = torch.stack(ref_offset_mask)
    ...
    data_dict['scanpath_x'] = scanpath_x
    data_dict['scanpath_y'] = scanpath_y
    data_dict['ref_encode'] = ref_encode
    data_dict['ref_offset_mask'] = ref_offset_mask

    return {'data': data_dict, 'data_samples': None}


def scanvla_collect_fn_LN(
        instances: Sequence[Dict],
        pad_index: int = DEFAULT_PAD_TOKEN_INDEX,
        return_hf_format: bool = False,
        use_varlen_attn: bool = False
    ...
    for example in instances:
        ...
        scanpath_x.append(example['fixation_x'])
        scanpath_y.append(example['fixation_y'])
        bbox.append(example['bbox'])
        unpredicted_mask.append(example['unpredicted_mask'])
        # ref_encode.append(example['ref_encode'])
        # ref_offset_mask.append(example['ref_offset_mask'])

    scanpath_x = torch.stack(scanpath_x)  #1
    scanpath_y = torch.stack(scanpath_y)
    unpredicted_mask = torch.stack(unpredicted_mask)
    ...
    data_dict['scanpath_x'] = scanpath_x
    data_dict['scanpath_y'] = scanpath_y
    data_dict['unpredicted_mask'] = unpredicted_mask
    data_dict['bbox'] = bbox
    # data_dict['ref_encode'] = ref_encode
    # data_dict['ref_offset_mask'] = ref_offset_mask
