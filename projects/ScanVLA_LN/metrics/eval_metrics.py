...
def get_metrics(predictions):
    clusters = np.load('/data/lyt/03-Repositories/02-others/01-Scanpath/ART3090/data/clusters_refcocogaze.npy', allow_pickle=True).item()
    gt = pickle.load(open('/data/lyt/03-Repositories/02-others/01-Scanpath/ART3090/data/refcocogaze_test_synced.pkl', mode='rb'))
    ...
    ans = get_seq_score(predictions, clusters=clusters)
    print(ans)
    print(cc_dict)
    print(nss_dict)

    ans['CC'] = cc_dict['CC']
    ans['NSS'] = nss_dict['NSS']
    return ans
