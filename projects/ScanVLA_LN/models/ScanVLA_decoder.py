...
class TransformerDecoderWrapper(nn.Module):
    ...
    def forward(self, 
        ...
        ref_len = vl_guidance_feats.size(0) # 先计算ref的长度，针对每个指代，扩充为bs=ref_len的大小

        pred_embeddings_vision = pred_embeddings_vision.expand(-1, ref_len, -1)

        # 查询信息
        query = self.with_pos_embed(torch.zeros(self.query_len, ref_len, self.hidden_dim, device=self.query_embed.weight.device, dtype=vl_guidance_feats.dtype), 
