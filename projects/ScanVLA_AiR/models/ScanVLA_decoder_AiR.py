...
class TransformerDecoderWrapper(nn.Module):
    def __init__(self, activation,                 
        ...
        self.query_embed_wrong = nn.Embedding(max_len, self.hidden_dim)
        ...
        #context embeddings
        self.xy_embed_fixed= PositionEmbeddingSine2d((16, 26), hidden_dim=self.hidden_dim, normalize=True, flatten = False).pos
        self.query_pos_embed_fixed = PositionEmbeddingSine1d(max_len,hidden_dim=self.hidden_dim, normalize=True).pos # nn.Embedding(self.pack_len, self.hidden_dim)

        self.xy_embed_fixed = self.xy_embed_fixed.reshape(-1, 1, hidden_dim).to(self.query_embed.weight.device)

        self._reset_parameters()

        self.dtype = torch.bfloat16
        self.to(self.dtype)
        self.xy_embed_fixed=self.xy_embed_fixed.to(self.query_embed.weight.dtype)
        self.query_pos_embed_fixed=self.query_pos_embed_fixed.to(self.query_embed.weight.dtype) 
    ...
    def forward(self, 
        ...
        if performance:
            ...
            # 查询信息
            query = self.with_pos_embed(torch.zeros(self.query_len, bs, self.hidden_dim, device=self.query_embed_wrong.weight.device, dtype=vl_guidance_feats.dtype), 
                                  pos=self.query_embed_wrong.weight.unsqueeze(1))

            # 查询信息和历史注视点编码和vl_guidance_feats融合
            querypos_embed = self.query_embed_wrong.weight.unsqueeze(1)
