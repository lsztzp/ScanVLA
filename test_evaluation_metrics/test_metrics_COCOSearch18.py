...
def run_model(model, tokenizer, task_name, Image_path, num_samples=1):
    ...
    with torch.no_grad():
        ...
        pred_embeddings_vl = pred_embeddings_vl[0]
        ...
        for tok, y, x, t in zip(token_type, ys_i, xs_i, ts_i):
            if tok == 0:
                scanpath.append([min(320-2, y.item()),min(512 - 2, x.item()), t.item()])
            else:
                break
...
def parse_args():
    ...
    parser.add_argument('--config', default='projects/ScanVLA_COCOSearch18/configs/ScanVLA_TP.py', help='config file name or path.')

    parser.add_argument('--max_len', type=int, default=7, help='save folder name')
    ...
    parser.add_argument('--task', default='cup', type=str, help="if evaluation is in ZeroGaze setting, the unseen target to evaluate the model")
    
    parser.add_argument('--pthmodel',default='pretrained/checkpoints/COCOSearch18_TP_SemSS_0550.pth', help='pth model file')

    parser.add_argument('--num_samples', default=1, type=int, help="Number of scanpaths sampled per test case")

    parser.add_argument('--condition', type=str, default='present', help='save folder name')
